...
# Global variable to store the optimized prompt
...
async def async_optimize():
    global optimized_prompt  # Declare the global variable
    temp_store_cm = AsyncPostgresStore.from_conn_string("postgresql://postgres:12345@localhost:5432/langmem")
    async with temp_store_cm as temp_store:
        await temp_store.setup()
        item = await temp_store.aget((instructions_id,), key=instructions_key)
        optimized_prompt = item.value["prompt"]  # Store the prompt in the global variable

asyncio.run(async_optimize())

# Now you can use optimized_prompt anywhere in your code
  ...
  description=f"""
    ...
    ***Instructions***
    ...
    Your task involves collecting two pieces of information from the user:
      1. Dispute ID (mandatory): Ask the user for the dispute ID. If the user provides the dispute ID in their input query, ask them to confirm it. The dispute ID should be a number. After collecting the dispute ID, ask the user to confirm it again. 
      If confirmed, use the dispute ID for the task.
